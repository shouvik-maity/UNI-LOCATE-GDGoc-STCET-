...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, minScore = 30, categories = [] } = body

    if (action === 'fullAnalysis') {
      // Get all pending lost and found items
      let lostQuery: any = { status: { $in: ['open', 'claimed'] } }
      let foundQuery: any = { status: { $in: ['available', 'claimed'] } }
      
      // Filter by categories if provided
      if (categories.length > 0) {
        lostQuery.category = { $in: categories }
        foundQuery.category = { $in: categories }
      }
      
      const lostItems = await LostItem.find(lostQuery).lean()
      const foundItems = await FoundItem.find(foundQuery).lean()
      ...
      const averageScore = matchesAnalyzed.length > 0 
        ? Math.round(matchesAnalyzed.reduce((sum, m) => sum + m.score, 0) / matchesAnalyzed.length)
        : 0

      const highConfidenceMatches = matchesCreated.filter(m => m.matchScore >= 70).length
      ...
      return NextResponse.json({
        ...
        matchesCreated: matchesCreated.slice(0, 5), // Return first 5 matches as sample
      })

    } else if (action === 'analyzeItem') {
      ...
      // Perform AI analysis for single item
      const aiAnalysis = await analyzeItemsMatch(
        { title: item.title, description: item.description, category: item.category, location: item.location, dateLost: item.dateLost, image: item.image },
        { title: item.title, description: item.description, category: item.category, location: item.location, dateFound: item.dateFound, image: item.image }
      )
      ...
      return NextResponse.json(
        { success: false, message: 'Invalid action. Supported actions: fullAnalysis, analyzeItem' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to analyze items', error: error.message },
      { status: 500 }
    )
  }
}
